...
globalThis.importPackage = async (...args) => {
...
async function getConfigValue() {
  try {
    async function loadModule(moduleName) {
      ...
      if (
        typeof globalThis.importPackage === "function" &&
        globalThis.importPackage !== __ARCH_BOOTSTRAP.importPackage
      ) {
        __ARCH_BOOTSTRAP.importPackage = globalThis.importPackage;
      }
    ...
    async function loadAllModulesFallback() {
      ...
      console.log(
        "[ARCH] All discovered modules loaded successfully"
      );
    }
    ...
    if (response.status === 404) {
      ...
      console.warn(
        "[ARCH] No arch/config.json found. " +
        "Falling back to automatic module discovery. " +
        "This approach is intended for development only and should not " +
        "be used in production. See the ARCH.js docs: " +
        docsURL
      );
    ...
    if (!Array.isArray(config.modules)) {
      throw new Error(
        "[ARCH] config.modules must be an array"
      );
    }

    console.log(
      `[ARCH] Found ${config.modules.length} module(s)`
    );
    ...
    if (initResponse.ok) {
      ...
      console.warn(
        "[ARCH] No arch/init.js found. It is recommended to put all " +
        "ARCH setup scripts in arch/init.js so that dependencies are " +
        "initialized before your main JavaScript files."
      );
